...
## 使用脚本前先将curl请求中的 eb86bd52-fe28-4f03-a944-60428823540e 改为你自己邀请链接的值,获取方式： curl -m 10 -sI "https://warp.plus/EmoCb" | grep -oE "referrer=.*$" | cut -f2 -d=
...
for((i = 0; i < ${flowdata}; i++)); do
    ...
    --data "{\"key\": \"$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 43)=\",\"install_id\": \"$install_id\",\"fcm_token\": \"APA91b$install_id$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 134)\",\"referrer\": \"241c8837-aa76-4b9a-8286-1b18b1eb7f14\",\"warp_enabled\": false,\"tos\": \"$(date -u +%FT%T.$(tr -dc '0-9' </dev/urandom | head -c 3)Z)\",\"type\": \"Android\",\"locale\": \"en_US\"}" \
